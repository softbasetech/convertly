...
const validateApiKey = async (req: Request, res: Response, next: Function) => {
  ...
  if (!user.isPro) {
    return res.status(403).json({ message: "This API requires a Pro subscription" });
  }
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/convert", isAuthenticated, upload.single("file"), validateFileUpload, async (req, res) => {
    try {
      ...
      try {
        ...
        // Image conversions
        else if (["jpg", "jpeg", "png", "webp"].includes(sourceFormat) && 
        ...
        // Serve the file
        res.download(convertedFilePath, conversion.convertedFilename, (err) => {
  ...
  app.post("/api/convert/api", validateApiKey, upload.single("file"), validateFileUpload, async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ message: `API conversion error: ${error.message}` });
    }
  });
  ...
  app.post("/api/qr-code", isAuthenticated, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: `QR Code generation error: ${error.message}` });
    }
  });
    ...
    app.post('/api/stripe-webhook', async (req, res) => {
      ...
      switch (event.type) {
        ...
        case 'customer.subscription.deleted':
          ...
          const subUser = allUsers.find(u => u.stripeCustomerId === cancelledCustomerId);
